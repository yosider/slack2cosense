...
const generateResponse = async (
  ...
  try {
    ...
    const formattedMessages =
      threadMessages
        .map((msg) => formatMessage(msg, team, channel))
        .join("\n") + "\n\n";
    let body = formattedMessages.replace(/[/?#\{}^|<>%\s\n]/g, (char) =>
      encodeURIComponent(char)
    );

    const threadTimeText = moment
      .unix(parseFloat(thread_ts.split(".")[0]))
      .tz("Asia/Tokyo")
      .format("YYYY-MM-DD HH:mm:ss");
    const urlBase = `https://cosen.se/${config.projectName}/${threadTimeText}?body=`;
    const maxBodyLen =
      config.maxBlockNumChar - urlBase.length - config.linkMessage.length - 3;

    // Split the body if it exceeds the maximum length
    const isMiddleOfEncodedChar = (str: string): boolean =>
      str.slice(-2).includes("%");
    const bodies: string[] = [];
...
const getThreadMessages = async (
  ...
  try {
    const response: AxiosResponse<SlackConversationRepliesResponse> =
      await axios.post(
    ...
    const userNames = await Promise.all(
      messages.map((msg: SlackMessage) => getUserName(msg.user))
    );
...
const getUserName = async (userId: string): Promise<string> => {
  ...
  try {
    ...
    if (!userResponse.data.ok) {
      console.warn(
        `Failed to get user info for ${userId}: ${userResponse.data.error}`
      );
      return "Unknown User";
    }

    const userName =
      userResponse.data.user?.real_name ||
...
const formatMessage = (
  ...
  const timeText = moment
    .unix(parseFloat(msg.ts.split(".")[0]))
    .tz("Asia/Tokyo")
    .format("YYYY-MM-DD HH:mm:ss");
  ...
  // Replace link format
  let processedText = msg.text.replace(
    /<(https:\/\/[^>|]+)(\|([^>]+))?>/g,
    (match, url, _, title) => {
